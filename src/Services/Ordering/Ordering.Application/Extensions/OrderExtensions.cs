...
{
    public static IEnumerable<OrderDto> ToOrderDtoList(this IEnumerable<Order> orders) => orders.Select(order => DtoFromOrder(order));
    
    public static OrderDto ToOrderDto(this Order order) => DtoFromOrder(order);

    private static OrderDto DtoFromOrder(Order order) => new OrderDto(
        order.Id.Value,
